...
import 'package:music_deewane/core/models/exported.dart' hide MediaItem;
import 'package:music_deewane/screens/screen/player_views/lyrics_search.dart';
...
import 'package:music_deewane/screens/screen/plugin_manager_screen.dart';
...
class _FullscreenLyricsViewState extends State<FullscreenLyricsView> {
  ...
  Timer? _lyricsTimeoutTimer;
  _LyricsTimeoutState _lyricsTimeoutState = _LyricsTimeoutState.none;
  @override
  void initState() {
    super.initState();
    ...
    _currentTrackId = playerCubit.player.currentTrackInfo.id;
  ...
  void _startLyricsTimeoutTimer() {
    _lyricsTimeoutTimer?.cancel();
    if (mounted) setState(() => _lyricsTimeoutState = _LyricsTimeoutState.none);
    Timer(const Duration(seconds: 8), () {
      if (mounted) setState(() => _lyricsTimeoutState = _LyricsTimeoutState.slow);
    });
    _lyricsTimeoutTimer = Timer(const Duration(seconds: 20), () {
      if (mounted) {
        setState(() => _lyricsTimeoutState = _LyricsTimeoutState.timedOut);
      }
    });
  }

  void _stopLyricsTimeoutTimer() {
    _lyricsTimeoutTimer?.cancel();
    if (mounted) setState(() => _lyricsTimeoutState = _LyricsTimeoutState.none);
  }
  ...
  void _openSettingsMenu(
      LyricsState state, MusicDeewanePlayerCubit playerCubit) {
    _hideControlsTimer?.cancel();
          ...
          children: [
                ...
                builder: (context, state) {
                  return AnimatedSwitcher(
                    ...
                    child: switch (state) {
                          ...
                          child: Column(
                            ...
                            children: [
                              const CircularProgressIndicator(
                                  color: Default_Theme.accentColor2),
                              if (_lyricsTimeoutState !=
                                  _LyricsTimeoutState.none) ...[
                                const SizedBox(height: 16),
                                Text(
                                  _lyricsTimeoutState ==
                                          _LyricsTimeoutState.slow
                                      ? l10n.lyricsSlowSearch
                                      : l10n.lyricsTimedOut,
                      ...
                      LyricsError() => _LyricsNotFoundView(
                          track: state.track,
                          onSearch: () {
            ...
            BlocListener<LyricsCubit, LyricsState>(
              listener: (context, state) {
                if (state is LyricsLoading) {
                  _startLyricsTimeoutTimer();
                } else {
                  _stopLyricsTimeoutTimer();
                }
            ...
            AnimatedPositioned(
              ...
              child: GestureDetector(
                onTap: () {},
                child: _buildTopBar(playerCubit, isDesktop),
              ),
            ),
            AnimatedPositioned(
              duration: const Duration(milliseconds: 400),
              ...
              child: GestureDetector(
                onTap: () {},
                child: _buildBottomControls(playerCubit, isDesktop),
              ),
            ),
            AnimatedPositioned(
              duration: const Duration(milliseconds: 400),
  ...
  Widget _buildBackground(MusicDeewanePlayerCubit playerCubit) {
  ...
  Widget _buildTopBar(MusicDeewanePlayerCubit playerCubit, bool isDesktop) {
    return Container(
          ...
          child: Row(
            ...
            children: [
              ...
              BlocBuilder<LyricsCubit, LyricsState>(
                builder: (context, state) {
                  return IconButton(
                    onPressed: () => _openSettingsMenu(state, playerCubit),
  ...
  Widget _buildBottomControls(
      MusicDeewanePlayerCubit playerCubit, bool isDesktop) {
    final l10n = AppLocalizations.of(context)!;
...
class _FullscreenSyncedLyricsState extends State<FullscreenSyncedLyrics> {
  ...
  void _forceSyncRecalculation() {
    final player = context.read<MusicDeewanePlayerCubit>().player.engine;
  ...
  void _setupPositionListener() {
    final playerCubit = context.read<MusicDeewanePlayerCubit>();
    final posStream = playerCubit.player.engine.positionStream;
...
class _LyricsNotFoundView extends StatelessWidget {
  final Track track;
  final VoidCallback onSearch;
  const _LyricsNotFoundView(
      {required this.track, required this.onSearch});
  @override
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    return Center(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              FilledButton.icon(
                onPressed: onSearch,
                icon: const Icon(MingCute.search_2_line, size: 18),
                label: Text(l10n.lyricsSearchManually),
                style: FilledButton.styleFrom(
                  backgroundColor: Default_Theme.accentColor2,
                  foregroundColor: Default_Theme.accentColor2dark,
                  padding:
              ...
              OutlinedButton.icon(
                ...
                icon: const Icon(MingCute.plugin_2_line, size: 18),
                label: Text(l10n.lyricsTryPlugin),
...
enum _LyricsTimeoutState { none, slow, timedOut }
