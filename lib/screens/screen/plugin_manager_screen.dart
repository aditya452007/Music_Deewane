...
import 'package:music_deewane/src/rust/api/plugin/manifest.dart';
import 'package:music_deewane/src/rust/api/plugin/plugin_info.dart';
import 'package:music_deewane/src/rust/api/plugin/types.dart';
...
import 'package:music_deewane/screens/screen/home_views/plugin_repository_view.dart';
import 'package:music_deewane/plugins/blocs/repository/plugin_repository_cubit.dart';
import 'package:music_deewane/plugins/utils/plugin_constants.dart';
...
class _PluginManagerScreenState extends State<PluginManagerScreen> {
  ...
  Widget _buildInstalledPluginsTab(
    ...
    return BlocConsumer<PluginBloc, PluginState>(
      ...
      listener: (context, state) {
        if (state.error != null) {
          final l10n = AppLocalizations.of(context)!;
          if (_lastFailedInstallPath != null &&
              state.error!.contains('Install failed')) {
            final path = _lastFailedInstallPath!;
            SnackbarService.showMessage(
              state.error!,
              action: SnackBarAction(
                label: l10n.pluginRetry,
                onPressed: () {
                  context.read<PluginBloc>().add(
                      InstallPlugin(packedFilePath: path, shouldLoad: true));
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    icon: const Icon(MingCute.download_2_line, size: 18),
                    label: Text(l10n.pluginEmptyInstall),
  ...
  Future<void> _installPlugin(BuildContext context) async {
    ...
    try {
      ...
      if (context.mounted) {
        _lastFailedInstallPath = filePath;
        context
            .read<PluginBloc>()
...
class _PluginDetailSheet extends StatelessWidget {
  ...
  void _confirmDelete(
    ...
    showMusicDeewaneDialog(
      ...
      actions: [
        MusicDeewaneDialogAction.text(l10n.pluginManagerCancel),
        MusicDeewaneDialogAction.filled(
