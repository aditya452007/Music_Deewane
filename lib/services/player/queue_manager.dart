...
/// The [MusicDeewanePlayer] reads [currentTrack] after calling navigation
...
class QueueManager {
  ...
  /// [MusicDeewanePlayer] checks this to skip the persistence listener
  ...
  ///
  /// Called from [MusicDeewanePlayer] via a throttled listener on
  ...
  Future<bool> restoreQueueState() async {
    try {
      ...
      final idx =
          (data['currentIndex'] as int?)?.clamp(0, tracks.length - 1) ?? 0;
      _isRestoring = true;
