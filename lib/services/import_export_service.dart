...
import 'package:music_deewane/services/db/legacy/legacy_media_id_mapper.dart';
import 'package:music_deewane/services/db/dao/track_dao.dart';
import 'package:music_deewane/services/m3u_processor.dart';
import 'package:music_deewane/services/storage_backup_service.dart';
...
class ImportExportService {
  ...
  static Future<String?> exportPlaylist(String playlistName,
    ...
    try {
      ...
      final Map<String, dynamic> playlistMap = {
        '_meta': {
          'generated_by':
              'Music Deewane - Open Source Music Streaming Application',
          'version':
              'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
          'exportedAt': DateTime.now().toIso8601String(),
          'format': 'v2',
          'note':
              'This file is automatically generated by Music Deewane and is intended solely for importing playlists within the application.',
        },
      ...
      final path = await writeToJSON(
          '${playlistDB.name}_MusicDeewanePlaylist.json', playlistMap,
          path: filePath);
  ...
  static Future<String?> exportTrack(TrackDB trackDB) async {
    try {
      ...
      final path =
          await writeToJSON('${trackDB.title}_MusicDeewaneSong.json', trackMap);
              ...
              actions: [
                ...
                FilledButton(
                  ...
                  child: Text(l10n.storageRestoreYes,
                      style: const TextStyle(
                          color: Default_Theme.accentColor2dark, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
          );
