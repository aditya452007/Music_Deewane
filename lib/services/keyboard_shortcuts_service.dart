...
class _KeyboardShortcutsHandlerState extends State<KeyboardShortcutsHandler> {
  ...
  bool _handleAltShortcut(LogicalKeyboardKey key) {
    ...
    if (key == LogicalKeyboardKey.arrowRight) {
      player.seekNSecForward(const Duration(seconds: 10));
      return true;
    } else if (key == LogicalKeyboardKey.arrowLeft) {
      player.seekNSecBackward(const Duration(seconds: 10));
      return true;
    }
  ...
  void _startVolumeAdjust(
      LogicalKeyboardKey key, double delta, MusicDeewanePlayer player) {
  ...
  void _togglePlayPause(MusicDeewanePlayer player) {
  ...
  double _changeVolume(MusicDeewanePlayer player, double delta) {
  ...
  (bool, double) _toggleMute(MusicDeewanePlayer player) {
    final currentVolume = player.engine.volume;
    if (currentVolume > 0) {
  ...
  LoopMode _cycleLoopMode(MusicDeewanePlayer player) {
  ...
  Future<void> _toggleLike(MusicDeewanePlayer player) async {
