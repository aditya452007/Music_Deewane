...
import 'package:music_deewane/core/constants/cache_keys.dart';
import 'package:music_deewane/repository/music_deewane/settings_repository.dart';
import 'package:music_deewane/services/player/stream_quality_selector.dart';
...
class SettingsCubit extends Cubit<SettingsState> {
  ...
  Future<void> _initSettings() async {
    final results = await Future.wait([
      ...
      // [27] musicLanguages
      _readSetting(
        () => _settingsRepo.getSettingStr(SettingKeys.musicLanguages,
            defaultValue: '[]'),
        SettingKeys.musicLanguages,
      ),
      // [28] favoriteArtists
      _readSetting(
        () => _settingsRepo.getSettingStr(SettingKeys.favoriteArtists,
            defaultValue: '[]'),
        SettingKeys.favoriteArtists,
      ),
    ]);
    ...
    // Parse music languages.
    List<String> musicLanguages = const [];
    try {
      final mj = results[27] as String?;
      if (mj != null && mj.isNotEmpty) {
        musicLanguages = (jsonDecode(mj) as List).cast<String>();
      }
    } catch (_) {}

    // Parse favorite artists.
    List<Map<String, String>> favoriteArtists = const [];
    try {
      final fj = results[28] as String?;
      if (fj != null && fj.isNotEmpty) {
        final decoded = jsonDecode(fj) as List;
        favoriteArtists =
  ...
  void setMusicLanguages(List<String> langs) {
    _settingsRepo.putSettingStr(SettingKeys.musicLanguages, jsonEncode(langs));
    emit(state.copyWith(musicLanguages: langs));
  }

  void setFavoriteArtists(List<Map<String, String>> artists) {
    _settingsRepo.putSettingStr(
        SettingKeys.favoriteArtists, jsonEncode(artists));
    emit(state.copyWith(favoriteArtists: artists));
  }
